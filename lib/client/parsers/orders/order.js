...
module.exports = (key, node) => ({
  ...
  paymentExecutionDetail: select(`${key}/orders:PaymentExecutionDetail/orders:PaymentExecutionDetailItem`, node).map(n => {
    return parsePaymentExecutionDetail('.', n)
  }),
  paymentMethod: parseStr(`${key}/orders:PaymentMethod`, node),
  paymentMethodDetails: select(`${key}/orders:PaymentMethodDetails/orders:PaymentMethodDetail`, node).map(n => {
  ...
  shippedByAmazonTfm: parseBool(`${key}/orders:ShippedByAmazonTFM`, node),
  tfmShipmentStatus: parseStr(`${key}/orders:TFMShipmentStatus`, node),
  cbaDisplayableShippingLabel: parseStr(`${key}/orders:CbaDisplayableShippingLabel`, node),
  ...
  earliestDeliveryDate: parseDate(`${key}/orders:LatestShipDate`, node),
  latestDeliveryDate: parseDate(`${key}/orders:LatestShipDate`, node),
