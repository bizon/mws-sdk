...
module.exports = (key, node) => ({
  numberOfOffers: select(`${key}/NumberOfOffers/OfferCount`, node).map(n => parseOfferCount('.', n)),
  lowestPrices: select(`${key}/LowestPrices/LowestPrice`, node).map(n => parseLowestPrice('.', n)),
  buyBoxPrices: select(`${key}/BuyBoxPrices/BuyBoxPrice`, node).map(n => parseBuyBoxPrice('.', n)),
  ...
  buyBoxEligibleOffers: select(`${key}/BuyBoxEligibleOffers/OfferCount`, node).map(n => parseOfferCount('.', n)),
