...
module.exports = (key, node) => ({
  ...
  debtRecoveryItemList: select(`${key}/finances:DebtRecoveryItemList/finances:DebtRecoveryItem`, node).map(n => {
    return parseDebtRecoveryItem('.', n)
  }),
  chargeInstrumentList: select(`${key}/finances:ChargeInstrumentList/finances:ChargeInstrument`, node).map(n => {
