...
class Client {
  ...
  get(resource, version, query, retry) {
  ...
  post(resource, version, body, retry) {
    const {region} = this.settings
  ...
  clearThrottlingRestores() {
    this.orders.throttlers.forEach(t => t.clearRestores())
    this.finances.throttlers.forEach(t => t.clearRestores())
  }
}
