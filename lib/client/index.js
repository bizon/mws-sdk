...
class MWSClient {
  constructor({
    ...
    if (!mwsRegion && !marketplaces) {
      throw new TypeError('Specify one of mwsRegion or marketplaces')
    }

    let sellerMarketplaces = []

    if (marketplaces) {
      sellerMarketplaces = getMarketplaces(marketplaces)
    }

    if (sellerMarketplaces.length === 0) {
      sellerMarketplaces = getMarketplacesFromRegion(mwsRegion)
    }
