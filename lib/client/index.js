...
const {cleanData, normalizeSearchParameters} = require('./utils')
...
class MWSClient {
  constructor({accessKeyId, secretAccessKey, sellerId, mwsToken, mwsRegion, marketplaces} = {}) {
  ...
  async get(resource, version, query, options) {
    ...
    try {
      return await apiClient.get(`https://${mwsDomain}${pathname}?${searchParameters}`, options)
    } catch (error) {
      if (error instanceof got.HTTPError && !(error instanceof MWSError)) {
