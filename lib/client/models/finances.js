...
const {dateToISOString} = require('../utils')
...
class Finances {
    ...
    const {body} = await this.client.post(
      ...
      hasToken
        ...
        : {
            ...
            PostedAfter: dateToISOString(postedAfter),
            PostedBefore: dateToISOString(postedBefore),
          },
      {
        retry: {
          retries: 30,
    ...
    const {body} = await this.client.post(
      ...
      hasToken
        ...
        : {
            ...
            MaxResultsPerPage: maxResultsPerPage,
            FinancialEventGroupStartedAfter: dateToISOString(financialEventGroupStartedAfter),
            FinancialEventGroupStartedBefore: dateToISOString(financialEventGroupStartedBefore),
          },
      {
        retry: {
          retries: 30,
  ...
  async [_getServiceStatus]() {
    ...
    return parseGetServiceStatusResponse('/finances:GetServiceStatusResponse', parseXml(body))
