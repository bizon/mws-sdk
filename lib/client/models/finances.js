...
class Finances {
  ...
  async [_listFinancialEvents]({
    ...
    const {body} = await this.client.post(RESOURCE, VERSION, hasToken ? {
      ...
      PostedAfter: parseDate(postedAfter),
      PostedBefore: parseDate(postedBefore)
    }, {
      retry: {
        retries: 30
      }
  ...
  async [_listFinancialEventGroups]({
    ...
    const {body} = await this.client.post(RESOURCE, VERSION, hasToken ? {
      ...
      MaxResultsPerPage: maxResultsPerPage,
      FinancialEventGroupStartedAfter: parseDate(financialEventGroupStartedAfter),
      FinancialEventGroupStartedBefore: parseDate(financialEventGroupStartedBefore)
    }, {
      retry: {
        retries: 30
      }
