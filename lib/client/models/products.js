...
class Products {
  constructor(client) {
    ...
    this.getProductCategoriesForSku = throttle(this[_getProductCategoriesForSku].bind(this), 1, 20, {
      amount: 1,
      interval: 5 * 1000
    })

    this.getProductCategoriesForAsin = throttle(this[_getProductCategoriesForAsin].bind(this), 1, 20, {
      amount: 1,
      interval: 5 * 1000
    })
  ...
  async [_getProductCategoriesForSku]({
    marketplaceId,
    sellerSku
  }) {
    const {body} = await this.client.post(RESOURCE, VERSION, {
  ...
  async [_getProductCategoriesForAsin]({
    marketplaceId,
    asin
  }) {
    const {body} = await this.client.post(RESOURCE, VERSION, {
