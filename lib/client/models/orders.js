...
class Orders {
  constructor(client) {
    ...
    const getOrdersThrottler = new Throttler(6, {
      amount: 1,
      interval: 60 * 1000
    })
    this.getOrders = getOrdersThrottler.proxy(this[_getOrders].bind(this))

    const listOrdersThrottler = new Throttler(6, {
      amount: 1,
      interval: 60 * 1000
    })
    this.listOrders = listOrdersThrottler.proxy(this[_listOrders].bind(this))

    const listOrderItemsThrottler = new Throttler(6, {
      amount: 1,
      interval: 2 * 1000
    })
    listOrderItemsThrottler.proxy(this[_listOrderItems].bind(this))

    this.throttlers = [getOrdersThrottler, listOrderItemsThrottler, listOrderItemsThrottler]
  }
