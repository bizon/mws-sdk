...
const {parseDate, arrayToObject} = require('../utils')
...
class Reports {
  ...
  async [_getReportRequestList]({
    ...
    const {body} = await this.client.post(RESOURCE, VERSION, hasToken ? {
      ...
      MaxCount: maxCount,
      RequestedFromDate: parseDate(requestedFromDate),
      RequestedToDate: parseDate(requestedToDate),
  ...
  async [_getReportList]({
    ...
    const {body} = await this.client.post(RESOURCE, VERSION, hasToken ? {
      ...
      AvailableFromDate: parseDate(availableFromDate),
      AvailableToDate: parseDate(availableToDate),
