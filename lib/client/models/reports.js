...
const {arrayToObject, dateToISOString, reportOptionsToString} = require('../utils')
...
class Reports {
  ...
  async [_requestReport]({reportType, startDate, endDate, marketplaceIdList, reportOptions}) {
  ...
  async [_getReport]({reportId, format}) {
    ...
    const raw = iconv.decode(response.body, charset)
